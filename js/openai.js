...
const ONI_CA_KEYWORD = '鬼CAモード';

function detectOniMode(targetCompany) {
  if (!targetCompany) return { isOni: false, cleanCompany: targetCompany };
  const isOni = targetCompany.includes(ONI_CA_KEYWORD);
  const cleanCompany = targetCompany.replace(ONI_CA_KEYWORD, '').trim();
...
async function generateCareerDocuments(params) {
  ...
  if (isOni) {
    parts.push('【🔥 鬼CAモード ON 🔥】');
    parts.push('辛口だが愛のあるキャリアアドバイスを出力してください。老後資金シミュレーション（financial_reality）は必須です。');
    parts.push('');
  }
  ...
  if (cleanCompany || targetPosition) {
    parts.push('【応募情報】');
    if (cleanCompany) parts.push(`応募先企業名: ${cleanCompany}`);
  ...
  if (isOni) {
    parts.push('');
    parts.push('【鬼CAモード追加要件】');
    parts.push('- career_advice.summary は辛口で、老後資金の問題にも必ず触れてください。');
    parts.push('- career_advice.financial_reality を必ず出力してください（老後資金シミュレーション）。');
    parts.push('- 求人提案は「やりたい仕事」ではなく「人生設計に有利な仕事」を優先してください。');
    parts.push('- growth_areas は遠慮せず本質的な課題を指摘してください。');
    parts.push('- recommendations は年収アップ・資産形成・スキルアップを最優先にしてください。');
  }
  ...
  try {
    ...
    const result = validateAndFillDefaults(parsed);
    result._isOniMode = isOni;
    return result;
